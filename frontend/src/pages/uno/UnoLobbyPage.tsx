...
import type { UnoParticipant, UnoSessionStatus } from "../../types";

type OpenSession = { id: string; status: UnoSessionStatus };
...
export default function UnoLobbyPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel("uno-sessions-watch")
      .on("postgres_changes", { event: "*", schema: "public", table: "uno_sessions" }, (payload) => {
  ...
  useEffect(() => {
    ...
    if (session.status !== "lobby") {
      sounds.sessionStart();
      navigate(`/uno/play/${session.id}`);
      return;
    }
    const channel = supabase
      .channel(`uno-lobby-watch-${session.id}`)
      .on("postgres_changes", { event: "*", schema: "public", table: "uno_participants", filter: `session_id=eq.${session.id}` }, () => loadRoster(session.id))
      .subscribe();
    return () => {
      supabase.removeChannel(channel);
    };
  ...
  async function handleJoin() {
    ...
    setBusy(true);
    setError(null);
    const { error } = await invokeFunction("uno-play", { action: "join_game", session_id: session.id });
    setBusy(false);
    ...
    <div className="app-shell">
      ...
      <div className="container container--narrow">
        <div className="card text-center">
            ...
            <>
              <p className="text-muted">
                {justEnded
                  ...
                  : "No UNO game happening right now. Keep an eye on Discord for the next announcement!"}
              </p>
          ...
          {session && !joined && (
            <>
              <p className="text-muted">A game is about to start — join before the host deals in.</p>
          ...
          {session && joined && (
            <>
              <div className="row" style={{ justifyContent: "center", margin: "16px 0" }}>
                <UnoCardView card={{ color: "wild", value: "wild" }} size="lg" />
              </div>
              <p className="text-muted">You're in! {roster.length} player{roster.length === 1 ? "" : "s"} seated.</p>
              ...
              <p className="hint">Waiting for the host to start the game… (2–10 players)</p>
            </>
          )}
        </div>
      </div>
