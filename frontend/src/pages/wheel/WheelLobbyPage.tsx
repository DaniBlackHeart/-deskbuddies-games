...
import { WHEEL_MAX_PLAYERS, WHEEL_MIN_PLAYERS } from "../../lib/wheelConstants";
import type { WheelParticipant, WheelSessionStatus } from "../../types";

type OpenSession = { id: string; status: WheelSessionStatus };
...
export default function WheelLobbyPage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "wheel_participants", filter: `session_id=eq.${session.id}` }, () => loadRoster(session.id))
      .subscribe();
    return () => {
      supabase.removeChannel(channel);
    };
